{
  ...
  # to the VPN-assigned address (pool has been seen as 172.16.17.32/28 and
  # 172.16.17.64/27, so match the whole /24) and tries to reach Discord's
  ...
  networking.firewall.extraCommands = ''
    iptables -A OUTPUT -s 172.16.17.0/24 ! -o tun0 -j REJECT
  '';
  networking.firewall.extraStopCommands = ''
    iptables -D OUTPUT -s 172.16.17.0/24 ! -o tun0 -j REJECT 2>/dev/null || true
  '';
}
